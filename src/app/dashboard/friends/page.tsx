...
import { searchByCode, sendRequest, getFriends, getRequests, respondRequest, cancelRequest } from "@/lib/friendsClient";
...
function RequestsPanel({
  ...
  open: boolean;
  onClose: () => void;
  items: { id: string; sender_id: string; receiver_id: string; status: "pending" | "accepted" | "rejected"; created_at: string }[];
  type: "incoming" | "sent";
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
      <div className="w-full max-w-lg rounded-xl border border-[#595959]/30 bg-[#161616] p-4 text-white shadow-xl">
        ...
        <div className="flex flex-col gap-2 max-h-[60vh] overflow-y-auto">
          ...
          {items.map((r) => (
            <div key={r.id} className="flex items-center justify-between rounded-lg bg-[#101010] p-3">
              <div className="text-sm text-white/80">ID: {r.id}</div>
              {type === "incoming" ? (
                <div className="flex gap-2">
                  <button className="px-3 py-1.5 rounded bg-white/10 hover:bg-white/20" onClick={() => onReject && onReject(r.id)}>Respinge</button>
                  <button className="px-3 py-1.5 rounded bg-[#2a7] hover:bg-[#2a7]/80" onClick={() => onAccept && onAccept(r.id)}>Acceptă</button>
                </div>
              ) : (
                <button className="px-3 py-1.5 rounded bg-red-600/80 hover:bg-red-600" onClick={() => onCancel && onCancel(r.id)}>Anulează</button>
              )}
            </div>
          ))}
        </div>
...
function FriendsUI() {
  type FriendUser = { id: string; username: string | null; display_name: string | null; avatar_url: string | null; friend_code: string | null; isVerified?: boolean };
  ...
  const [incoming, setIncoming] = useState<{ id: string; sender_id: string; receiver_id: string; status: "pending" | "accepted" | "rejected"; created_at: string }[]>([]);
  const [outgoing, setOutgoing] = useState<{ id: string; sender_id: string; receiver_id: string; status: "pending" | "accepted" | "rejected"; created_at: string }[]>([]);
  ...
  return (
    <div className="flex flex-col ">
      <section className="rounded-xl p-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={async () => { await refreshRequests(); setShowIncoming(true); }}
          >
            Incoming
          </button>
          <button
            className="px-4 py-2 rounded-2xl bg-white/10 hover:bg-white/20 text-white text-sm whitespace-nowrap"
            onClick={async () => { await refreshRequests(); setShowSent(true); }}
          >
            Sent
          </button>
        </div>
      ...
      <RequestsPanel
        ...
        items={incoming}
        type="incoming"
        onAccept={async (id) => { await respondRequest(id, "accept"); await refreshRequests(); }}
        onReject={async (id) => { await respondRequest(id, "reject"); await refreshRequests(); }}
      />
      ...
      <RequestsPanel
        ...
        items={outgoing}
        type="sent"
        onCancel={async (id) => { await cancelRequest(id); await refreshRequests(); }}
      />
    </div>
  );
}
